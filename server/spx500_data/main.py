...
def get_collection(collection_name: str = SPX500_COLLECTION_NAME):
...
def create_nested_relations():
    spx_insert_list = []
    for spx in get_collection().find():
        new_detail_doc = {
              "sub_industry": spx.get("sub_industry", ""), "headquarters": spx.get("headquarters", ""),
              "date_added": spx.get("date_added", ""), "cik": spx.get("cik", ""),
              "founded_date": spx.get("founded_date", ""), "net_worth": spx.get("net_worth", ""),
              "details": spx.get("details", ""),
              "details_tr": spx.get("details_tr", "")
        }

        new_doc = {
            "_id": spx.get("_id"),
            "symbol": spx.get("symbol"),
            "security": spx.get("security", ""),
            "sector": spx.get("sector", ""),
            "details": new_detail_doc
        }

        spx_insert_list.append(InsertOne(new_doc))

    try:
        get_collection("SPX500_new").bulk_write(spx_insert_list)
    except Exception as e :
